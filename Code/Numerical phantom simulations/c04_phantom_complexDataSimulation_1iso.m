...
% the 1-mm isotropic numerical head phantom with fixed M0, T2* and
...
% With and without realistic noise levels and a phase offset
...
phase_phi0 = zeros([matrix_size n_echoes]);
magnitude = zeros([matrix_size n_echoes]);
...
complexSignal = magnitude .* exp(1i * phase_phi0);
...
magnitude_withNoise = abs(complexSignal_withNoise);
phase_phi0_withNoise = angle(complexSignal_withNoise);

% Wrap phase without noise
phase_phi0_wr = wrapToPi(phase_phi0);
...
save('magnitude_wholeHead_1iso.mat', 'magnitude_withNoise','magnitude')
save('phase_wholeHead_1iso.mat', 'phase_phi0_withNoise','phase_phi0_wr')
